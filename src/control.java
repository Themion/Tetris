...
public class control {
	...
	int state(int block, int dir) { 
		int ret = this.center[dir] + this.add[block][dir]; 
		
		if((dir == 1) && (ret < 0)) return 0;
		else return ret;
	}
	...
	{
		for(int i = 0; i < 3; i++) {
			if( (this.state(i, 0) < 0) || (this.state(i, 0) > 9) || (this.state(i, 1) > 19) ) return false;
		}
